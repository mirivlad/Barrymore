...
// Rule описывает режим так, как о нём должен говорить сам Бэрримор.
//
// Отличается от Describe: тот объясняет режим владельцу, а этот кладётся
// в промпт вторым лицом. Разные адресаты — разные слова, но одно правило:
// сказать неправду о собственном поведении Бэрримор не должен ни тому,
// ни другому.
func (p Policy) Rule() string {
	switch p.Mode {
	case ModeAsk:
		return "Ничего не записывай сам: каждую запись владелец подтверждает отдельно."
	case ModeAuto:
		return "Сказанное владельцем и следующее из сказанного ты записываешь сам. " +
			"Чувствительное всё равно выносится на его решение."
	default:
		return "Сказанное владельцем прямо ты записываешь сам, без лишних вопросов. " +
