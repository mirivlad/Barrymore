...
func (i Identity) SystemPrompt(sections []ContextSection, now time.Time) string {
	...
	b.WriteString("  Для каждой записи укажи `sensitivity` и `confidence`: очевидное о владельце,\n")
	b.WriteString("  сказанное им прямо, ты запишешь сам; чувствительное и неуверенное будет\n")
	b.WriteString("  вынесено на его решение. Если владелец прямо просит что-то запомнить —\n")
	b.WriteString("  предложи это с высокой уверенностью;\n")
