...
## Дополнение 2026-08-05: умения, обучение, подключение

### Маршруты

| Маршрут | Что делает |
|---|---|
| `GET /api/v1/skills` | умения, их применения, записи опыта и предложения освоить новое |
| `POST /api/v1/skills/{id}/apply` | применить умение; ответ возвращается в разговор и в нить |
| `POST /api/v1/skills/{id}/retire` | снять умение с применения, с обязательной причиной |
| `POST /api/v1/skills/learn` | освоить предложенный порядок действий |
| `POST /api/v1/harness/study` | изучить незнакомый инструмент по имени команды |
| `POST /api/v1/harness/adopt` | принять его в штат по решению владельца |

### События

| Событие | Смысл |
|---|---|
| `skill.run.completed` | умение применено; в payload — шаги, наблюдения и цена |
| `skill.learned` | освоено новое умение |
| `skill.retired` | способ снят с применения, с причиной |
| `practice.outcome.noted` | исход применения способа |
| `practice.stale` | способ признан негодным |
| `harness.surveyed` | незнакомый инструмент опрошен безопасными пробами |
| `harness.adopted` | инструмент принят в штат |
| `harness.refused` | предложенный способ запуска отвергнут проверкой |

События подключения — аудиторские: штат ведёт собственные проекции, и вторая
запись о том же исполнителе только расходилась бы с первой.

### Контракт ответа модели

`own_actions` — что Бэрримор берётся сделать сам: идентификатор умения из
показанного ему списка, каталог и причина. Придуманное умение отвергается,
и отказ виден владельцу.

Правило §7 («свободный shell_command запрещён») распространяется и сюда,
и сюда же — на подключение инструментов: способ запуска, предложенный моделью,
принимается только если каждый его аргумент дословно встречается в справке,
напечатанной самим инструментом.
