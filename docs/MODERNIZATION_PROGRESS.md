...
## Stage 5 — durable live turn progress

Состояние: **кодовый, restart/rebuild и живой браузерный gates подтверждены**.

Разговорный POST теперь создаёт долговечный `TurnRun`, возвращает `202 Accepted`
и передаёт исполнение приложению, а не контексту HTTP-запроса. Реплика владельца
и queued-ход записываются атомарно. Частичный unique index и domain error
обеспечивают не больше одного активного хода на разговор; другие разговоры
могут выполняться параллельно.

Реализовано:

- event-sourced lifecycle `queued → running → completed|failed|interrupted` и
  projection, восстанавливаемая из журнала;
- runtime-owned стадии от recall до finalization;
- безопасный streaming OpenAI-compatible provider: фрагменты структурированного
  JSON накапливаются приватно, наружу идут только счётчики, а предложение
  публикуется лишь после полной сборки и валидации;
- ограниченный latest-value broker для частой внутрипроцессной телеметрии без
  события журнала на каждый фрагмент;
- чтение активного и конкретного хода, корреляция SSE по `turn_id` и
  восстановление UI после reload;
- одна динамическая строка с текущим действием, временем, приблизительными
  live-счётчиками и точными финальными provider timings;
- аварийная граница runner: panic становится `failed`, shutdown прекращает
  приём новой работы, а оставшиеся после рестарта ходы становятся
  `interrupted` без неявного повторного запуска.

Живой gate 2026-08-21 выполнен на локальной Ornith через настоящий браузер.
Во время ответа строка дошла до `≈675 токенов · ≈8.0 ток/с · 84 с`; финал
показал `10375 вход · 792 выход · 14.2 ток/с · 90 с`. Повторный сценарий с
перезагрузкой во время работы восстановил `16 с` вместо сброса таймера; в
узком окне 390 px строка осталась однострочной, а поле ввода — заблокированным.
Финальная реплика появилась ровно один раз с `10888 вход · 823 выход ·
14.3 ток/с · 92 с`; 👍, реплика и метрики пережили ещё одну перезагрузку.

Автоматические gates включают отдельный restart/projection-rebuild сценарий,
focused Go и Node tests и полный `make ci`. Ограничения: отмены хода и общей
очереди пока нет; приблизительные live-токены выводятся из объёма накопленного
текста; после рестарта процесса незавершённая работа не продолжается. Интерфейс
не добавляет анимацию и остаётся под существующим глобальным
`prefers-reduced-motion`, но отдельная browser-эмуляция этого режима не
проводилась.

Срез не реализует Capability Acquisition. Следующий шаг — исследование,
владелец-одобренное создание адаптера, проверка и превращение успешного пути в
Procedure/опыт без заранее прошитой зависимости от конкретного устройства.
